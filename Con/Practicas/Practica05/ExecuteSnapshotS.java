...
public class ExecuteSnapshotS {

	public static void main(String[] args) {
		int capacity = 4;
		String init = null;
		ConcurrentLinkedQueue<Integer> queue = new ConcurrentLinkedQueue<>();// Implementacion linealizable de una cola no acotada
		SimpleSnapshotH<String> snapshotI = new SimpleSnapshotH<String>(capacity,init);
		SimpleSnapshotH<String> snapshotR = new SimpleSnapshotH<String>(capacity,init);
			...
			for(int i = 0; i < 10; i++) {
				...
				executor.execute(new RunnableS(ntask, queue,numRand,snapshotI,snapshotR)); 
			}
			executor.shutdown();
			...
			System.out.println("Ejecutanto el SimpleSnapshotH");
